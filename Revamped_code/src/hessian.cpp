...
// I am going to refactor this function into mini-functions that are easy to understand and test:
...
std::vector<double> calculate_frequencies(const vec& eigenvalues)
{
    ...
    for (int i = 0; i < eigenvalues.n_elem; ++i)
    {
        ...
        {
            //freq_cm = -std::sqrt(std::abs(frequency)) * AU_TO_WAVENUMBER;
            freq_cm = 0.0;
        }
        frequencies.push_back(freq_cm);
    }

    // Optional: Print results immediately
    std::cout << "\n=== Corrected Frequencies ===\n";
    for(size_t k=0; k<frequencies.size(); ++k) {
        std::cout << "Mode " << k << ": " << frequencies[k] << " cm^-1" << std::endl;
    }
