                       ...
                       const std::vector<ContractedShell>& basis_set_shells)
{
    ...
    for (size_t i = 0; i < atoms.size(); ++i)
    {
        ...
        {
            if (shell.atomic_num != atom.atomic_number) continue;
    ...
    for (const auto& atom : atoms)
    {
        ...
        for (const ContractedShell* shell : s_shells)
        {
            basis_functions.push_back({atom.x, atom.y, atom.z, 0, 0, 0, atom.atomic_number, shell});
        }

        // Then the p shell(s)
        for (const ContractedShell* shell : p_shells)
        {
            basis_functions.push_back({atom.x, atom.y, atom.z, 1, 0, 0, atom.atomic_number, shell});
            basis_functions.push_back({atom.x, atom.y, atom.z, 0, 1, 0, atom.atomic_number, shell});
            basis_functions.push_back({atom.x, atom.y, atom.z, 0, 0, 1, atom.atomic_number, shell});
        }
    }
    */
...
std::pair<mat,vec> solve_eigenvalue_problem(const mat& S, const mat& H)
{
    ...
    //L_inv_sqrt.diag(0).print("\nS Eigen Values Sqrt Inverse");
    ...
    //C_prime.print("\nH_script Eigen Vectors");
    //E.print("\nH_script Eigen Values");
